...
/**
 * Recalcula las alertas del mes y la quincena actuales.
 *
 * El navegador no controla el mensaje ni los
 * destinatarios. Toda la información se obtiene
...
/**
 * Evita que GET dispare las notificaciones.
 */
...
/**
 * Reclama temporalmente una alerta usando una
 * transacción. Evita que dos llamadas simultáneas
 * envíen la misma notificación.
 */
...
> {
  ...
  await getAdminDb().runTransaction(
    async (transaction) => {
      const snapshot =
      ...
      /**
       * Si el uso baja de 90%, se reinicia
       * completamente el ciclo de alertas.
       */
      if (
        nivel === "normal"
      ) {
      ...
      /**
       * Otra solicitud ya está enviando esta alerta.
       */
      if (
        leaseActivo
      ) {
      ...
      /**
       * No repetir alertas ya entregadas:
       *
       * 90 → 90: no enviar.
       * 100 → 100: no enviar.
...
async function cargarFidsActivos(): Promise<
  string[]
> {
  const snapshot =
    await getAdminDb()
...
async function cargarLimites(): Promise<
  LimitesVariables
> {
  const snapshot =
    await getAdminDb()
...
/**
 * La aplicación guarda fecha como una cadena ISO.
 */
...
function obtenerRangoBusqueda(
  ...
  /**
   * Margen para cubrir la diferencia entre
   * UTC y America/New_York.
   */
  const margen =
    36 *
    60 *
...
/**
 * Solo acepta llamadas POST iniciadas desde
 * la misma aplicación.
 */
