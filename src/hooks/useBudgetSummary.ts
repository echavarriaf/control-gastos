...
export function useBudgetSummary({
  ...
  return useMemo(() => {
    const gastosMes = gastos.filter(
      (gasto) =>
        obtenerPeriodoDesdeISO(gasto.fecha) ===
        mesSeleccionado,
    );

    const pagosMes = pagos.filter(
      (pago) =>
        obtenerPeriodoDesdeISO(pago.fecha) ===
        mesSeleccionado,
    );
    ...
    const gastosQuincena = gastosMes.filter(
      (gasto) =>
        obtenerQuincenaDesdeISO(gasto.fecha) ===
        quincenaSeleccionada,
    );

    const pagosQuincena = pagosMes.filter(
      (pago) =>
        obtenerQuincenaDesdeISO(pago.fecha) ===
        quincenaSeleccionada,
    );
    ...
    const limiteVariableQuincenal =
      CATEGORIA_KEYS.reduce(
        (total, key) =>
          total + limites[key].quincenal,
        0,
      );

    const totalPlanMensual =
      totalFijo + limiteVariableMensual;
    ...
    const saldoVariableMes = Math.max(
      totalGastosMes - totalPagosMes,
      0,
    );
    ...
    const totalGastosQuincena =
      gastosQuincena.reduce(
        (total, gasto) => total + gasto.monto,
        0,
      );

    const totalPagosQuincena =
      pagosQuincena.reduce(
        (total, pago) => total + pago.monto,
        0,
      );
    ...
    const porcentajeFijoPagado = porcentaje(
      totalPagadoFijoMes,
      totalFijo,
    );
    ...
    const resumenCategorias: ResumenCategoria[] =
      CATEGORIA_KEYS.map((key) => {
        ...
        const saldoMes = Math.max(
          gastosCategoriaMes -
            pagosCategoriaMes,
          0,
        );

        const gastosCategoriaQuincena =
          gastosQuincena
            .filter(
              (gasto) =>
                gasto.categoria === key,
        ...
        const saldoQuincena = Math.max(
          gastosCategoriaQuincena -
            pagosCategoriaQuincena,
          0,
        );
        ...
        return {
          key,
          saldoMes,
          disponibleMes: Math.max(
            limites[key].mensual - saldoMes,
            0,
          ),
    ...
    const resumenFijos: ResumenFijo[] =
    ...
    [...gastos, ...pagos, ...pagosFijos].forEach(
      (registro) => {
        const periodo =
          obtenerPeriodoDesdeISO(
            registro.fecha,
